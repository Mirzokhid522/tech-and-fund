...
# Set cache to 12 hours (43200 seconds) or 24 hours (86400 seconds) 
# This ensures Render queries Yahoo Finance strictly 1-2 times a day.
          ...
          for prop_name, prop_data in props.items():
            ...
            if p_type == "title":
              ...
              if p_type == "rollup":
                ...
                if (
                    "number" in rollup_obj
                    and rollup_obj["number"] is not None
                ):
                ...
                if (
                    "number" in formula_obj
                    and formula_obj["number"] is not None
                ):
...
def calculate_technical_scores():
  ...
  # Pass full desktop browser headers into requests session for yfinance
  ...
  try:
    # Download all symbols in one single bulk call using the custom session
    data = yf.download(
        yf_symbols,
        period="1y",
        interval="1d",
    ...
    if data.empty:
      raise Exception("Empty response or rate limit triggered.")
    ...
    for symbol in PAIRS:
      ...
      try:
        ...
        score = 0.0
        score += 1.0 if current_price > ma20 else -1.0
        score += 1.5 if ma20 > ma50 else -1.5
        score += 2.0 if ma50 > ma100 else -2.0
        score += 2.5 if ma100 > ma200 else -2.5
    ...
    print(f"[DEBUG] Bulk download blocked or failed: {e}")
    # Fallback to previously cached data if available so the app never breaks
    if cache["data"]:
      return {item["symbol"]: item["tech_score"] for item in cache["data"]}
    for symbol in PAIRS:
      tech_scores[symbol] = 0.0
